package org.axgl {
	...
	public class Ax extends Sprite {
		 ...
		 * global touch points
		 */ 
		public static var tuch:AxPoint = new AxPoint()
		/**
		...
		public static var touch:Array = new Array;
		//public static var tuch:AxPoint = new AxPoint();
		//public static var touchBoundary:AxRect = new AxRect();
		//public static var touchJustHappened:Boolean;
		/**
		...
		protected function onTouchBegin(event:TouchEvent):void {
			...
			/*if (touchBoundary.contains(event.stageX,event.stageY))
			{
				tuch.x = event.stageX;
				tuch.y = event.stageY;
		...
		protected function onTouchMove(event:TouchEvent):void {
			...
			{
				if (event.touchPointID == touch[i]._id)
				{
					touch[i]._x = event.stageX;
					touch[i]._y = event.stageY;
					//touch[i]._t = 2;
				}
			}
		}
